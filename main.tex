...
\WarningFilter{latex}{Size substitutions}
...
\title{The Red Queen Runs on LLMs: Taxonomy, Evaluation, and Co-Evolution of LLM-Driven Cybersecurity}
...
\begin{abstract}
Large language models stopped answering security questions and started doing security work.
We track that shift across 36 records (academic papers, competition artifacts, industry
disclosures, and registry entries) plotted on two axes: who chooses the next action
(A0 through A3, from prompted assistant to autonomous production hunter), and where
in the offense/defense loop the system operates (B1 through B4). Twelve records
received autonomy assessment (eleven definitive, one unresolved) plus two general-SE comparators. Pipeline automation (A1) is demonstrated at ecosystem scale in OSS-Fuzz.
Task agents (A2) show up in seeded production research, entry-point-assisted auditing,
black-box platform operation, and DARPA's AI Cyber Challenge scored rounds. Nothing
in the corpus meets our A3 definition. We also trace how offense and defense feed
each other, documenting one clean cross-program link (Project Zero's SQLite campaign
citing AIxCC as inspiration) while refusing to generalize to field-wide co-evolution.
Evaluation validity turns out to be a measurable constraint: information conditions swing
reported exploitation rates by more than an order of magnitude, dataset reconstruction
collapses detection scores, and proof-of-vulnerability oracles let teams game the metric
without fixing the bug. We end with a roadmap held to its own falsifiers, from
autonomy-versus-scale ablations to decision-impact audits aimed at making an
increasingly autonomous practice also a more verifiable one.
\end{abstract}
...
\label{sec:intro}
Something changed between 2024 and 2026. Vulnerability discovery, exploitation, and patching
acquired a new kind of operator: software systems where large language models choose and
execute security-relevant actions. Google's agent found an exploitable memory-safety zero-day
in SQLite that 150 CPU-hours of AFL on a CLI target (corpus pre-seeded with required keywords) could not rediscover, a harness-coverage-bound result in a single seeded variant-analysis run where authors note a target-specific fuzzer would likely be as effective (see Sec.~\ref{sec:discovery})~\cite{bigsleep2024}. A commercial
agent reached \#1 on HackerOne's US leaderboard (June 2025, vendor-reported)~\cite{xbowtop12025}.
A government challenge scored seven competition CRSs designed to operate autonomously under
the competition's no-human-interaction rules on finding, proving, and
patching vulnerabilities in open-source software~\cite{tobbuttercup2025,darparesults2025,r7sokaixcc2026}.
Anthropic reported, as a single-source threat-intelligence assessment, an intrusion
campaign whose tactical execution was 80--90\% autonomous with human target selection and perhaps 4--6 critical decision points (authorization at exploitation/exfiltration) though the system hallucinated credentials and remains an obstacle to fully autonomous operation~\cite{anthropicgtg2025,anthropicgtgreport2025}. These events
share no benchmark and no metric. That is exactly why existing surveys cannot answer the
questions they raise.

\subsection{The Empirical Problem}
Three problems block straightforward assessment. First, \emph{unit confusion}: results are
reported about models (benchmark pass rates), pipelines (fuzz-target generation), agents
(tool-using loops), and organizations (leaderboard ranks) as if they measure the same thing.
They do not. Second, \emph{loop fragmentation}: discovery, validation, and repair get evaluated
separately even when deployed systems integrate them. Third, \emph{validity opacity}: headline
numbers travel without the information conditions, label provenance, or adjudication rules
that produced them. GPT-4-based agents exploited 86.7\% of one-day CVEs at pass@5 (40\% overall) when given their
descriptions and 7\% when not on n=15, while all other tested LLMs scored 0\% at pass@5 with descriptions and ZAP and Metasploit scored 0\% where applicable (several vulns not amenable to scanners, which cannot autonomously exploit)~\cite{fang2024agents}, yet the qualified number is the one
usually quoted.

\subsection{Our Approach}
We build a PRISMA-informed evidence base from three streams (academic literature, competition and
industry documents; registry records) and score every record on two axes. The first is a
\emph{decision-authority} gradient A0--A3, assigned strictly by documented behavior: who
chooses the next action? Autonomy claims get graded by their weakest missing condition. The
second is the offense/defense loop positions B1--B4, recorded as sets for systems that span
phases. Eleven in-window records receive definitive autonomy classifications; one remains
unresolved; two general-SE comparators and the remaining 22 provide context, validity evidence, loop-transition
documentation, or positioning against prior work (18 context +2 loop-transition +2 background anchors, see Table). We keep the distinction explicit throughout.
...
a documented causal edge from AIxCC to Project Zero's research
program (Sec.~\ref{sec:coevolution}).
\textbf{C3.} A systematization of evaluation-validity
failures as measurable phenomena, with a five-part contamination-control protocol
(Sec.~\ref{sec:validity}).
\textbf{C4.} A falsifier-specified experimental roadmap
(Sec.~\ref{sec:roadmap}).

\subsection{Insights}
\textbf{I1.} Evaluation, deployment, and discourse increasingly measure progress by
agentic autonomy rather than model capability alone; whether this reflects a real shift
in underlying capability remains unresolved.
\textbf{I2.} Multiple documented
...
\textbf{I3.} Evaluation validity is a binding methodological constraint on
what this field can claim to know, pending the decision-impact audit (E7) that would
...
verified results everywhere. No included evidence met our operational definition of A3. We include
Agentless~\cite{agentless2024} only as a security-relevant repair baseline where its fixed
three-stage architecture (localization, repair, and validation) is directly compared with
agentic vulnerability repair systems; it is classified as A1 because its workflow follows a
predetermined sequence rather than letting the model choose what to do next. General-purpose systems such as SWE-agent~\cite{sweagent2024} are included only as non-security comparators (SWE-bench) and do not carry a security-task autonomy level.
...
their security-relevant evaluations started with secure-coding benchmarks~\cite{csev12023bg} and
...
a critique Risse and B\"ohme systematized via a survey of 81 ML-for-vulnerability-detection papers and an empirical study of context-dependence and spurious correlations~\cite{rissebohme-wrongexam2024}. Ullah et
al.~\cite{secllmholmes2025} found early on that LLMs cannot reliably identify and reason about
security vulnerabilities, the baseline skepticism our validity section extends. Li et
al.~\cite{iris2025} show that IRIS (LLM for specification inference plus CodeQL taint analysis and LLM contextual filtering) detects 28 more vulnerabilities than CodeQL alone on CWE-Bench-Java.
\emph{Interactive agent harnesses} descend from execution-feedback benchmarks like InterCode-CTF
(pre-2024 background)~\cite{intercode2023} through NYU CTF Bench~\cite{nyuctfbench2024} and
Cybench~\cite{cybench2024}. PentestGPT introduced the penetration-testing tree (PTT) as a
planning data structure for LLM-empowered automated penetration testing, reporting gains over naive LLM prompting in its evaluation~\cite{pentestgpt2024}. The Model Context Protocol ecosystem, measured at
scale for server-side insecurity~\cite{mcpfirstlook2025} and analyzed for tool-poisoning,
spec-level, and governance risks~\cite{mcppoisoning2026,mcpspec2026,mcpgovernance2025}, supplies
the tool-integration substrate on which A2 autonomy operates.

\subsection{Systematizations We Position Against}
Table~\ref{tab:positioning} codes the seven closest prior efforts. R1's SLR mapped
300+ LLM$\times$security works but closed before the agentic turn~\cite{r1slr2024}; R2
covers detection-to-remediation with no exploitation axis~\cite{r2slr2024}; R3, the
closest survey chronologically, organizes pentesting only, treats ``co-evolution'' as an
architecture--benchmark relationship internal to that subfield, and excludes competition-grade
systems by corpus design; it also falls outside our window (July 2026), so we treat it as a
comparator, not migration evidence~\cite{r3survey2026}. R4 is detection-centric within
software security~\cite{r4csur2025}. R5 systematizes the security of agent systems (agents
as attack surface) whereas our subject is agents as operators of the vulnerability
loop~\cite{r5usenix26agentic}. R6's TOSEM SLR contributes 263 detection studies through
November 2025 but carries no offense axis and no industrial track~\cite{r6tosem2026}. R7, the AIxCC
...
Chen et al.~\cite{chen3dtaxonomy2026} independently synthesize 39 frontier cybersecurity
benchmarks across discovery, auditing, repair, and threat intelligence, finding a systemic
gap between static benchmark success and end-to-end adversarial efficacy. Their taxonomy
provides an independent benchmark-level complement to our security-scoped autonomy axis rather
than a replacement for record-level decision-authority classification. Halder et
al.~\cite{seclens2026} show that aggregate benchmark scores obscure stakeholder-dependent
conclusions: their multi-stakeholder evaluation framework demonstrates that the same model can
score 31 points apart depending on whether a CISO, engineering leader, or security researcher
lens is applied, confirming that model selection for vulnerability detection is not a
single-objective problem.

\begin{table}[t]
\centering\scriptsize
\caption{Our comparative coding of prior systematizations. C=covered, P=partial, A=absent under the definitions used in this study.}
...
\midrule
R1 \cite{r1slr2024}        & Dec'24 & P & C & A & A & P\\
R2 \cite{r2slr2024} & Dec'24 & A & C & A & A & P\\
R3 \cite{r3survey2026}  & Jul'26$^{\dagger}$ & C & P & P (intra-pentest) & A & A\\
R4 \cite{r4csur2025}        & '25    & A & C & A & A & A\\
R5 \cite{r5usenix26agentic}    & '26    & P & P & A & P & A\\
R6 \cite{r6tosem2026}         & Nov'25 & A & C & A & P (repo) & P\\
R7 \cite{r7sokaixcc2026}       & Feb'26 preprint$^{\ddagger}$ & C & C & P & C & A\\
...
\\
\multicolumn{7}{l}{$^{\ddagger}$First public preprint is in-window; later publication metadata do not extend the cutoff.}
\end{tabular}}
\end{table}

\subsection{What Remains Uncovered}
We found no prior systematization that joins (i) an autonomy gradient applied to documented behavior across
...
We conduct a PRISMA-informed systematization with one deliberate structural change: because the
subject spans academic papers, competition artifacts, vendor disclosures, and registry records,
we maintain \textbf{three evidence streams} rather than forcing everything through a single
review flow. Stream~A (academic literature) follows the classical funnel; Stream~B
(competition/industry/foundation documents) follows targeted acquisition with quote-level
extraction; Stream~C (registry records, e.g., NVD entries) follows existence verification. The
streams get reported separately and summed only at the top level.

\subsection{Search Strategy}
Stream~A searches used arXiv boolean queries (eight formulations over ``large language model''
/ ``language model'' / ``LLM agent'' / ``AI agent'' crossed with vulnerability discovery, detection,
penetration testing, CTF, exploit, program repair, cyber reasoning system, and vulnerability; total 973 raw hits)
...
query log with per-query hit counts is Appendix~B. Temporal scope follows a record's first
public appearance rather than the date of a later revision or venue publication; where later
versions fall outside the cutoff, they are used only for bibliographic context unless the
specific evidentiary claim is independently supported by an in-window or primary source.
...
surveys positioned via verified metadata, 10 competition/industry primaries with verbatim
quote extraction, and 1 registry record. Of 339 candidates (316 distinct arXiv IDs plus seed/chaining records), 303 were held out (see Fig.~\ref{fig:prisma}): wrong identifier (1), press-only claims (1), out-of-window scope including background demotions ($\sim$280), unresolvable primary URL (1), and near-duplicates ($\sim$20). Saturation was tested via the last 50 ArXiv in-window queries returning zero novel records plus DBLP and citation-chaining snowball; collection stopped when those channels yielded no new inclusions.

\textbf{Eligibility of general-purpose systems.} A system originally designed for general
software engineering qualifies when the evaluated artifact applies it to a security-specific task: vulnerability discovery, exploit
generation, or security patching. The autonomy level reflects the security evaluation, not
the system's general capability. The security variant CodeRover-S (record ossfuzz-aifixing-2024) is classified A2; the general AutoCodeRover record remains n/a context. This rule ensures
the corpus captures the security-relevant frontier without conflating general SWE
capability with security-specific deployment.
...
plus evidence class (benchmark, controlled experiment, live enterprise-network experiment,
competition, production discovery, incident report, measurement, registry), unit type, access
status, and provenance type (PDF+TXT, HTML-extract, metadata-only).
Table~\ref{tab:rubric} defines the scoring dimensions.
Vendor-reported statistics
are treated as source-reported unless independently reproduced; third-party reproduction passes
are distinguished explicitly where available.

\begin{table}[t]
\centering
\scriptsize
\caption{Quality rubric scoring definitions.}
\label{tab:rubric}
\begin{tabular}{@{}clll@{}}
\toprule
Score & Repr. & Baseline rigor & Artifact avail. \\
\midrule
0 & No attempt & No comparison & None \\
1 & Author-reported & Informal & Partial (code only) \\
2 & Third-party attempt & Formal, caveats & Full + instructions \\
3 & Independent reproducing & Rigorous + ablation & Persistent archive \\
\bottomrule
\end{tabular}
\end{table}
...
with A3 also requiring independent planning, execution, validation, and reporting in
...
uses the set $\{$B1..B4$\}$, recorded as sets for multi-phase systems. The artifact
(\texttt{evidence/autonomy-loop-assignments.csv}) publishes the record-level assignment fields and
classification outputs used for the frozen matrix;
benchmarks carry probed phases rather than axis positions, keeping thematic coverage out of the
taxonomy.

\subsection{Limitations}
First-page screening caps per query, publisher-API absence, and English-language sources are
 documented limitations; the bounded A3 finding remains stable within the documented corpus under the sensitivity scenarios reported in Sec.~\ref{sec:framework}, but cannot exclude undocumented systems outside the observable evidence base.
 Classification stability: all 36 records were coded for unit type, temporal scope, access,
 evidence, and loop role; the autonomy$\times$loop taxonomy (Sec.~\ref{sec:framework}) was
 assigned to the 12 autonomy-assessed records (plus two general-SE comparators held as context). As an author-reported within-coder check (no independent artifact), we
 repeated the classification for all 12 autonomy-assessed records, with six boundary
 cases (GTG-1002, XBOW, ARTEMIS, Big Sleep, Argusee, AIxCC cluster) receiving additional
 review during initial coding and a second pass. The two passes yielded consistent
 classifications, though intermediate uncertainty concentrated on environment-class
 boundaries (E1 vs.\ E2); GTG-1002 remains unresolved because the available incident
 documentation cannot support a definitive autonomy level. The
 weakest-missing-condition rule preserved that uncertainty and did not change any
 assigned record's autonomy level. We report this as a within-coder stability observation; a formal inter-rater
 study with independent domain-expert coders remains future work. Additional methodological constraints include:
 ...
 assignment fields in the artifact CSV, (ii) by explicitly distinguishing source-reported from
 independently reproduced evidence where available, and
...
execution, validation, and reporting (\emph{all four verbs}) in environment class E2, i.e., 
...
(production-like enterprise-scale university network, 12 subnets/${\sim}8{,}000$ hosts under study safeguards with IT awareness and manual approval of flagged actions; E1/E2 boundary, see Limitations). Two corollaries do heavy lifting:
...
GTG-1002 incident remains \emph{unresolved} rather than A3, where evidence is insufficient
to assign a definitive autonomy level.
...
empty under the operational definition; B4 exists only as loop-transition documentation tied to
 documented systems. Counts are reconciled against the assignment CSV and the record inventory;
 the table is a frozen audit view rather than a generated data product.

\input{figures/autonomy-loop-map}
...
competition (AIxCC finals, August 2025), and live enterprise-network experimental settings (ARTEMIS).
...
scored. The 2026 record adds consolidation: three in-window systematizations (plus one out-of-window comparator), an infrastructure-risk
cluster around agent tool protocols, and the first institutionalized B4 documentation identified in our corpus.

\textbf{A3 robustness.} No included evidence met our operational definition of A3. The sensitivity analysis makes the counterfactuals explicit: with independent
planning/target selection ($P$), execution ($X$), validation ($V$), reporting ($R$), and an E2
target ($E$), the baseline predicate is $P\land X\land V\land R\land E$. Relaxing $R$ alone
admits XBOW, while relaxing $P$ alone admits ARTEMIS and relaxing $E$ alone admits none;
relaxing $P$, $R$, and $E$ together admits XBOW, the AIxCC CRS cluster, and ARTEMIS under
that altered rule. These are counterfactual
qualifiers, not A3 findings, and the core conclusion remains bounded to the strict definition.
The weakest-missing-condition rule prevents a system from qualifying under the strict rule merely
because of a headline autonomy claim; the remaining blockers are documented per candidate in
Appendix~\ref{app:sensitivity}.

% tables/autonomy-loop-matrix.tex — frozen audit view (12 autonomy-assessed +2 general-SE / 2 background /
% 2 loop-transition-doc / 18 context). Source of truth available in this repository:
% evidence/autonomy-loop-assignments.csv plus the record inventory in the appendix.
\begin{table*}[t]
\centering
...
sweagent2024             & 2024 & in & n/a        & context: general SE comparator (SWE-bench) \\
autocoderoever-2024      & 2024 & in & n/a        & context: general SE; security variant is CodeRover-S below \\
ossfuzz-aifixing-2024    & 2024 & in & A2        & $\{B3\}$ (CodeRover-S) \\
...
intercode2023bg          & 2023 & bg & n/a        & $\{B2\}^p$ \\
csev12023bg              & 2023 & bg & n/a        & $\{B1\}^p$ \\
\midrule
...
\midrule
\multicolumn{5}{l}{\emph{(iv) Off-axis / contextual / out-of-window records (18)}}\\
...
\bottomrule
\multicolumn{5}{l}{\scriptsize Totals: 12 autonomy-assessed ($A1{\times}2$, $A2{\times}9$, uncertain${\times}1$)
$+$ 2 general-SE context $+$ 2 background $+$ 2 loop-transition $+$ 18 context $=$ 36 records.}\\
\end{tabular}
}
\end{table*}
...
OSS-Fuzz program. Its LLM executes the first four steps of the developer fuzzing workflow, namely 
...
roadmap item, anchoring its A1 classification~\cite{ossfuzz-levelingup-2024}. The reported yield is
...
existing fuzz targets written by humans''~\cite{ossfuzz-levelingup-2024}. Two features matter for our
...
announcement time~\cite{ossfuzz-levelingup-2024}.
...
a pool of recent commits using Gemini~1.5~Pro~\cite{bigsleep2024}. Three
...
the result is experimental and that ``it's likely that a target-specific fuzzer would be at least as effective''
in general~\cite{bigsleep2024}. The built-in baseline is instructive: AFL failed to
rediscover the bug after 150 CPU-hours on a CLI target even with a corpus pre-seeded with the required keywords,
pointing to harness and environment coverage as an important limiting factor; the 150 CPU-hour result does not isolate compute from harness design~\cite{bigsleep2024}.
No CVE was assigned: the bug was fixed pre-release~\cite{bigsleep2024}.
...
LSP-based code reader, and DARKNAVY reports that Argusee discovered and analyzed CVE-2025-37891, a vendor-characterized arbitrary
kernel heap overflow in the Linux USB MIDI2-to-UMP conversion path, alongside fifteen
vendor-reported flaws in medium projects (GPAC, GIFLIB), separate from the kernel CVE~\cite{darknavyargusee2025}. Its own
...
were resolved and 303 triaged, with 208 duplicates and 209 informative outcomes (plus 33 new, 125 pending, 36 not-applicable per dashboard), roughly 39
percent duplicate/informative among several outcome categories rather than new/resolved
...
taxonomy is the platform rule: ``All findings were fully automated, though our security team reviewed them pre-submission to comply with
...
The ARTEMIS study was first posted as an arXiv preprint in December 2025; the local evidence
copy is v2 dated March 2026. It places seven agent instances (six existing scaffolds plus ARTEMIS) against ten professionals on the same
large research-university enterprise network (${\sim}8{,}000$ hosts, 12 subnets), scoring the
first ten hours of sixteen-hour runs~\cite{artemis2025}. We use it as environment-class E2
evidence that agents can operate on realistic infrastructure; comparative performance figures
are treated separately because the study's evaluation design does not isolate general agent
capability from scaffold and task conditions. Liu et al.~\cite{agora2026} extend the A2
paradigm to consensus-protocol auditing, where LLM agents autonomously detect bugs in
four consensus implementations (Raft, EPaxos, HotStuff, BullShark) including production-level etcd/sui via hypothesis-driven synthesis and iterative test generation, a domain-specific demonstration that A2 task-agent patterns generalize beyond
...
workflow, including seed selection, target selection, entry-point specification, and final reporting, rather
...
validity flags use the V1--V5 taxonomy of Sec.~\ref{sec:validity}. Operational statistics are
source-reported; their provenance varies across vendor disclosures, research reports, and
...
\midrule
OSS-Fuzz AI \cite{ossfuzz-levelingup-2024} & A1/E2 & report review & 26 vulns; CVE-2024-9143 &
 prior harnesses & V5 triage counts\\
Big Sleep \cite{bigsleep2024} & A2/E2 & curated seeds & n=1 find + repro &
...
XBOW \cite{xbowtop12025} & A2/E2 & mandated review & funnel stats; rank & leaderboard &
 unaudited dashboard\\
ARTEMIS \cite{artemis2025} & A2/E2 & comparison arm & first-10h score & professionals &
 single environment\\
\bottomrule
\end{tabular}}
\end{table}
...
instrument whose rules, incentives, and aftermath document how AIxCC redefined the unit of progress within its competition from vulnerability discovery toward integrated discovery--proof--patching (I1). Beyond competition settings, recent benchmarks such as
CVE-Bench~\cite{cvebench2025} evaluate LLM agents on real-world web application vulnerabilities,
reporting that agent frameworks exploit up to 13\% (12.5\% in primary results) of critical-severity CVEs in
sandboxed environments under the benchmark's one-day setting with five attempts, where a high-level vulnerability
description is supplied to the agent~\cite{cvebench2025}. Information conditions can change exploitation outcomes sharply in separate controlled experiments~\cite{fang2024agents}. Multi-agent red-teaming frameworks like Co-RedTeam~\cite{coredteam2026}
...
security benchmarks; those exploit tasks can provide the target codebase together with a description
of the specific vulnerability to be reproduced. CyberGym~\cite{cybergym2026} further scales evaluation to real-world
...
OpenAI, the challenge funneled 42 semifinal entrants down to seven finalists, each
...
(6 vs.\ 2 points at the top of the 1--2 vs.\ 3--6 range per the systematization)~\cite{darpascoring2025,taasc2024,r7sokaixcc2026}, making B3 capability the rational investment; and accuracy
modifiers penalized incorrect patches nonlinearly; Team Atlanta reports its PoV-validated
91.27\% patch accuracy yielding a modifier of $1-(1-0.9127)^4=0.9999$ and reports Theori's
PoV-free $44.4\%$ as $0.9044$~\cite{taafc2025}. Applying the displayed formula independently
to $0.4444$ gives $0.9047$, so we preserve $0.9044$ only as a source-reported value. The
scoring mathematics thus priced
patch-quality adjudication directly into strategy.

\subsection{What the Final Round Measured}
Trail of Bits describes its scored workload as 48 challenges across 23 open-source
repositories (a subset of the 63 final challenges per DARPA's official closeout)~\cite{tobbuttercup2025}; the later R7 systematization
accounts for 48 challenge projects across 24 repositories~\cite{r7sokaixcc2026}. We retain each figure within its respective source's accounting scope. The CRSs also surfaced real,
...
closeout reports the remaining aggregates: competitors' systems discovered 54 unique synthetic
vulnerabilities across 63 final challenges (86\%) and patched 43 (68\%), alongside 11 patches for the real
...
multilanguage, patching, SARIF), ensemble fuzzing and concolic execution alongside LLM agents~\cite{r7sokaixcc2026},
and an explicit integration taxonomy (LLM-augmented, LLM-opinionated, LLM-driven)~\cite{taafc2025} with
smaller models often outperforming larger or reasoning models for patch tasks; its
...
deterministic, expertise-decomposed workflows with LLMs only where classical tools fail~\cite{r7sokaixcc2026},
using exclusively less expensive, non-reasoning models, achieving lower reported cost per competition point at
\$181 per point (versus Atlanta's \$263) on 28 vulnerabilities and 19 patches across 20 CWE
classes with ${>}90\%$ accuracy as claimed by the participant~\cite{tobbuttercup2025}. \emph{Theori}
demonstrated purely static, PoV-free patching, trading accuracy for robustness to absent
proofs~\cite{taafc2025}. Team-level compute costs are consistent across our two
...
A prominent lesson in the winning team's account is a string-matching heuristic: Atlantis skipped
...
failure in one does not cascade; ensembling was explicitly framed as the engineering
answer to LLM non-determinism (what the systematization summarizes as hallucination versus non-determinism)~\cite{r7sokaixcc2026}. Buttercup concentrates expertise in deterministic
decomposition, integrating LLMs only where traditional tools fall short~\cite{r7sokaixcc2026}, augmenting
libFuzzer and Jazzer with LLM-generated test cases behind a multi-agent patcher with
separation of concerns, and attributed its efficiency to this architecture rather than model
scale: \$181 per competition point using exclusively non-reasoning models, versus \$263
...
Two quantitative facts refine any capability narrative. First, smaller models ``often
outperformed larger foundation models and even reasoning
...
patch takes ten-plus minutes, so Atlantis ran six patching agents (sixty is illustrative spare capacity, not a sourced figure), 
illustrating that, in this architecture, build-and-test throughput became a practical bottleneck
on B3 scaling~\cite{taafc2025}.
...
in follow-up commit b651084~\cite{taasc2024}. Discovery and repair here were performed by the system.
...
the strongest documented autonomy evidence in our corpus. It does not satisfy the paper's A3
...
context to request next~\cite{autocoderoever-2024}. Agentless deliberately inverts the design
...
vulnerabilities and states the distinction explicitly: ``We note that having autonomy in the LLM agent is
useful for successful security patching, as opposed to approaches like Agentless where the
control flow is fixed''~\cite{ossfuzz-aifixing-2024}. Its mechanism satisfies our test at the action
level; the model invokes AST search, file navigation, compilation, and test-execution tools,
gathering failure information before committing patches. Reported efficacy
on real OSS-Fuzz vulnerabilities remains far from saturation (52.4\% on a curated set under the
paper's conditions)~\cite{ossfuzz-aifixing-2024}, and validation is sanitizer/test-based rather than exploit-aware, 
precisely the gap AIxCC's oracle-gaming taxonomy names. CVE-Genie reproduces approximately 51\%
of CVEs published June 2024--May 2025 (428 of 841) with verifiable exploits at an average cost of \$2.77 per
CVE~\cite{cvegenie2025}, demonstrating the feasibility of automated vulnerability reproduction
at scale. Recent surveys map the broader scope of LLM-based automated program
repair~\cite{llmaprsurvey2026}, identifying four paradigms: fixed-workflow, single-agent,
multi-agent, and general-purpose code agents. On the security-specific repair side, Tree-of-Thought prompting plus AST-based context retrieval on sanitizer logs achieves substantially higher patch correctness than direct generation on
real-world vulnerability datasets (31/39 vs.\ 21/39 Zero-shot; Tree-of-Thought alone 24/39)~\cite{logsinpatchesout2025}, suggesting that structured
reasoning plus retrieval over repair candidates rather than single-shot generation is an important
design choice for B3 systems. LLM4CVE~\cite{llm4cve2025} extends this line with an iterative
pipeline with automated feedback (compiler and CodeBLEU) and LoRA fine-tuning on C critical-system code, requiring minimal expert intervention.
Beyond competition settings, Nong et al.~\cite{appatch2025} demonstrate that vulnerability
semantics reasoning combined with adaptive prompting enables automated patching of zero-day
vulnerabilities without test inputs or exploit evidence, achieving up to 28.33\% F1 (and 182\% recall) improvement
over the best baseline (including non-LLM SOTA) on 97 zero-day vulnerabilities. These systems establish that B3 capability now
spans from pipeline-orchestrated repair (Agentless) through iterative agentic workflows
(AutoCodeRover, CodeRover-S), yet validation of semantic security correctness beyond replaying known exploit inputs remains
unevenly evaluated across the literature. Two further findings qualify this picture. Wu et
al.~\cite{onebug2025} demonstrate how a known patched instance can be abstracted into a
recurring error pattern and searched at repository scale: across the Linux kernel their system
surfaced more than 22{,}000 candidate issues, with 246 of 400 manually inspected samples
confirmed as valid issues; separately, their recurring-pattern identification evaluation reports
92.2\% precision under its own evaluation protocol, showing that variant analysis scales further
than the n=1 result our A2 discussion initially suggests. Meng et al.~\cite{llmagentsbugfixing2026} provide a systematic comparison of
LLM-based bug-fixing systems, finding that fault-localization accuracy (especially at the code-symbol level) and correct issue reproduction are major determinants of repair success; their cross-system analysis shows varied returns, implying that additional
agentic interaction does not uniformly improve outcomes (synthesis), a finding with direct implications
for the B3 autonomy gradient.
Adjacent discovery-audit systems such as RepoAudit~\cite{repoaudit2025} demonstrate that
comparable agentic control patterns also extend upstream into repository-level vulnerability
analysis. OpenAnt~\cite{openant2026} combines repository-scale decomposition with adversarial
verification and dynamic testing, providing another example of a verification loop applied to
repository-scale vulnerability analysis. Li et al.~\cite{projectscale2026} provide the first
comprehensive empirical comparison of five LLM-based detectors against traditional static
analyzers at project scale, finding that while LLM-based tools uncover unique vulnerabilities
missed by traditional tools, both approaches suffer from very high false discovery rates, with
shallow interprocedural reasoning and misidentified source/sink pairs as primary failure modes, 
a result that directly qualifies the A2 systems in our corpus, which operate at the same
project-scale detection level but have not been independently evaluated against this
failure profile.
Charoenwet et al.~\cite{agenticscr2026} extend this line with AgenticSCR, an autonomous
agentic secure code review system for immature vulnerabilities that combines LLM reasoning
with static analysis tools in a pre-integration setting, demonstrating that agentic
code review can provide earlier security feedback than traditional deferred assessment.
...
patching emerged as a prominent mitigation strategy among the AIxCC architectures for LLM non-determinism; Atlantis capped its
patching agents at six not by principle but because validating one nginx patch takes ten-plus
minutes~\cite{taafc2025}, an infrastructure constraint on B3 scaling that benchmark-level success
rates do not capture.

\subsection{Blind Spots}
Five gaps bound current evidence. (i)~\emph{Semantic exploit-aware validation}: several
systems, including CodeRover-S and PatchEval-style evaluations, replay known exploit inputs
or PoCs against candidate patches. What remains sparsely documented outside the AIxCC
ecosystem is broader validation that tests whether exploitability has been eliminated beyond
the originally observed proof, for example through alternate exploit inputs, semantic analysis,
or adversarial mutation~\cite{ossfuzz-aifixing-2024,patcheval2025}.
(ii)~\emph{Adversarial inputs to repair}: Przymus et al.~\cite{advbugreports2026} demonstrate
that adversaries can craft bug reports causing LLM-based repair systems to introduce new
vulnerabilities while ostensibly fixing the reported bug: in their evaluation, 90\% of crafted
reports triggered attacker-aligned patches, while existing input and output defenses remained
substantially incomplete, a trust-boundary violation the manuscript's B3 discussion does not address.
(iii)~\emph{Regression and redeployment}: B4 exists in our corpus solely as foundation-documented
...
never as an academic evaluation target with regression metrics. (iv)~\emph{Attacker
adaptation}: no included work studies adversaries reacting to deployed auto-patchers; the
offensive side of B3 is absent. (v)~\emph{Interaction with discovery}: patching and discovery
...
us to use it for our testing''~\cite{bigsleep2024}. This is the corpus's cleanest causal
...
created transition incentives of up to \$200K per team to integrate CRSs into critical software~\cite{tobbuttercup2025};
...
defender agentic publicity: Anthropic reports (as a single-source threat-intelligence assessment) that GTG-1002 used Claude Code for 80--90\% of an
...
\$29.5M denotes the cumulative program prize commitment announced in 2023~\cite{darpa2023aixcc}, \$8.5M denotes the
...
past the scoreboard. DARPA and ARPA-H created transition incentives of up to \$200K per finalist team to integrate CRSs
...
retaining strategic authorization at critical escalation points~\cite{anthropicgtg2025,anthropicgtgreport2025}. The
incident remains an unresolved, single-source autonomy assessment. Its
...
supplied, the agent autonomously exploited 86.7\% at pass@5 (40\% overall); without it, success fell to 7\%, while every
baseline (GPT-3.5, eight open-source models, ZAP, Metasploit) scored zero with descriptions and was not retested without (implied zero)~\cite{fang2024agents}. This more-than-an-order-of-magnitude difference is produced by a single
supplied-knowledge variable, the supplied CVE text can function as a substantial information advantage, so any
capability claim inherits its information condition. We therefore report such results only with
their conditions attached, and the same discipline applies to temporal leakage: the benchmark
authors report that a substantial subset of vulnerabilities post-dated the relevant model
knowledge cutoff, yet supplied CVE descriptions still dramatically altered performance.

\subsection{V1/V5: Dataset-Level Invalidity, Measured by Reconstruction}
PrimeVul demonstrates that widely used portions of the vulnerability-detection benchmark layer were structurally
fragile: label noise, near-duplicate pairs across splits, and pre-split public exposure
...
models on the wrong exam~\cite{rissebohme-wrongexam2024}. The corpus also contains a live saturation
...
behavior only for the tested proof; teams documented mitigation-gaming patches, specifically recompiling
...
one (\$29.5M cumulative commitment announced in 2023~\cite{darpa2023aixcc}, \$8.5M final pool, \$30.5M
distributed~\cite{darpascoring2025,openssfaixcc2026}); these figures reflect different accounting scopes. These are administrative record
...
audit}: a second scorer re-labels a random 10\% sample with agreement statistics. We found no
included evaluation that satisfies all five requirements today; E3 (Sec.~\ref{sec:roadmap}) operationalizes the
 gap. On the false-positive front, Xiong and Zhang~\cite{siftingnoise2026} demonstrate that
LLM-based filtering can reduce initial false-positive rates from over 92\% on OWASP Benchmark aggregated SAST alerts to as low as 6.3\%
in the best configuration (SWE-agent + Claude Sonnet 4), though aggressive filtering can itself suppress true
vulnerabilities, suggesting that the contamination-control protocol's adjudication step (v)
could be augmented with LLM-assisted triage, provided the false-negative trade-off is explicitly
measured.
...
demonstrating that the supplied information condition materially determines performance in
this evaluation. PrimeVul's reconstruction moves StarCoder2 7B from 68.26\% F1 on BigVul to 3.09\% on its paired,
chronologically split evaluation, with GPT-3.5- and GPT-4-class prompting ``akin to random
guessing'' under the most stringent settings~\cite{primevul2024}, showing that dataset
construction and evaluation methodology can become a binding constraint on apparent model
performance. And XBOW's funnel makes
...
findings, so leaderboard position aggregates discovery quality with program-policy and outcome-definition effects
unless category-aware accounting is applied~\cite{xbowtop12025}. Three further findings
deepen the validity picture. Tang et al.~\cite{vultriage2026} demonstrate that triple-path
context augmentation improves LLM-based vulnerability detection, suggesting that input
engineering, not just model selection, materially affects reported performance.
Camarato et al.~\cite{promptaudit2026} show that prompt sensitivity accounts for significant
variance in LLM-based vulnerability detection, raising questions about reproducibility when
prompts are not standardized. And Huang et al.~\cite{semantictrap2026} investigate whether
fine-tuned LLMs actually understand vulnerability semantics or merely pattern-match surface
features, finding evidence for the latter, a result that qualifies this paper's
assumption that fine-tuning transfers genuine security understanding. Complementary
mechanistic analysis by Sandoval et al.~\cite{surgicalrepair2026} suggests a
Format-Reliability Gap: the same models that generate insecure code correctly identify
and explain the vulnerability when asked, with security representations remaining
computationally inert until the final layer where format-compliance demands compete
with them, consistent with an interpretability-level failure rather than a simple training-data deficit. Shahriar et
al.~\cite{wordsspeaklouder2026} further show that cognitive heuristics---halo, framing,
and anchoring---systematically affect LLM vulnerability
 detection, with their results suggesting that contextual cues can redirect detection
 away from actual vulnerabilities. Each datum predates or
 operates independently of the others; their convergence on the same methodological conclusion
 is why we treat validity as measured rather than alleged. The real-world gap is now quantified
 by independent work: Neef et al.~\cite{webvulndetect2026} report LLM detection rates of
 35--63\% on real-world WordPress plugin vulnerabilities, with substantial variance across
 models, while Dahiya et al.~\cite{frontierllmsready2026} show that Gemini 3.1 Pro reaches
81.3\% accuracy (89.7\% recall) on white-box function-level vulnerability classification but only 4--8\% on
 black-box ground-truth coverage without methodology guidance, confirming that benchmark
 performance overstates real-world capability when methodology and tooling scaffolds are absent.
...
\textbf{Publication and visibility bias.} Our industry stream consists of organizations that
...
invisible, so the bounded A3 finding is a statement about documented evidence, not about private
...
rarely generate blog posts. XBOW's published funnel (208 duplicates, 209 informative) illustrates
the problem, while the corpus does not expose negative results that may have remained unpublished
elsewhere.

\textbf{Source-quality asymmetry.} Academic records generally provide methods sections; some
vendor records are limited to dashboard-style reporting. We mitigate with quote-level extraction,
cross-source corroboration where possible (AIxCC cost tables agree across participant and
systematization sources), and explicit source-provenance wording, but asymmetry remains, and
one unresolved, vendor-reported incident (GTG-1002) remains a boundary-case limitation.
...
construction; coverage leans on arXiv, DBLP, official indexes, and citation chaining. Our
observable corpus is therefore concentrated on those channels, although the resulting coverage may not represent the field uniformly,
...
and by the weakest missing condition, and we publish the record-level assignment fields in
the artifact CSV; an independent inter-rater study was not conducted, which we flag as a limitation.
...
\textbf{Single-corpus induction.} All quantitative distributions (12 autonomy-assessed records,
including 11 classified and one unresolved, plus two general-SE comparators; per-cell counts) derive from one team's inclusion decisions under one protocol; the saturation
...
Controls: identical tools, prompt budgets, and scoring; release-pinned checkpoints.
...
Gap: public-solution contamination is widely hypothesized but insufficiently quantified for multi-step
agents, and long-horizon security tasks, multi-step discovery chains requiring sustained
context, remain highly sensitive to model, project, and configuration: in the May 2026
preprint v1 (183 validated vulnerabilities; local source is v2 with 344), Lee et al.~\cite{secbenchpro2026} evaluate across V8 and
SpiderMonkey, with the strongest frontier configuration (Codex GPT-5.5) reaching 47.6\% on V8 and 44.2\% on
SpiderMonkey (58.4\% overall), while the open-weight Kimi-K2.5 baseline reaches 1.9\% on V8 and 2.9\% on SpiderMonkey.
Union of Codex GPT-5.5 and Claude Code on V8 resolves 51 of 103 tasks (49.5\%) and on SpiderMonkey 55 of 104 (52.9\%) (local calculation on v2).
Setup: time-split evaluation on challenges disclosed after each model's cutoff, against
...
Gap: ``binding constraint'' lacks a documented case of a decision changing on
...
on a 36-record multi-stream corpus of which 12 records underwent autonomy assessment (11
definitive classifications, one unresolved) plus two general-SE comparators, is that
the evaluated systems, incentives, and discourse represented in our corpus increasingly measure progress in \emph{agentic
...
pipeline automation (A1) is demonstrated at ecosystem scale with humans at review boundaries; task agents
...
A3 autonomous production hunter. No included evidence met our operational definition of A3. The nearest candidate is an offensive incident,
...
variant-analysis campaign~\cite{bigsleep2024}; a maintainer extended an AI-found bug's fix to sibling code paths~\cite{taasc2024};
AIxCC generated documented institutional follow-through: DARPA created transition incentives
of up to \$200K per finalist team for transition activities~\cite{tobbuttercup2025}, while the competition itself carried prize commitments reported
as \$29.5M in DARPA's 2023 announcement~\cite{darpa2023aixcc} and \$30.5M in OpenSSF's later retrospective~\cite{openssfaixcc2026}; separately,
OpenSSF established governance and hosting structures for post-competition CRS development~\cite{openssfaixcc2026}; and
threat intelligence seeded a pre-exploitation disclosure (vendor-asserted TI premise and foiling counterfactual unverifiable, cited as documented disclosure process)~\cite{bloggoogle2025}. These documented edges do not license
the arms-race vernacular: shared-cause explanations survive globally, and attacker-side
response remains unmeasured.

\appendices

\textbf{On I3 (observed; binding claim pending E7).} Validity failures here are not rhetorical.
Information conditions change reported exploitation rates by more than an order of magnitude~\cite{fang2024agents}; dataset
reconstruction collapses detection SOTA~\cite{primevul2024}; oracle gaming has a named taxonomy~\cite{taafc2025}; environment-specific
heuristics can materially affect system outcomes~\cite{taafc2025}; and leaderboard counting conflates outcome categories~\cite{xbowtop12025}. Because
these measures allocate prizes and rank vendors directly and inform funding
decisions through transition programs, evaluation validity is plausibly a binding
methodological constraint on what this field can claim to know — stands if E7 finds documented reversals, and is capped at important constraint otherwise (E7 falsifier).
...
% appendix-a.tex - frozen record inventory; reconcile against evidence/autonomy-loop-assignments.csv
\vspace{6pt}
...
provenance. Record-level assignment fields ship in \texttt{evidence/autonomy-loop-assignments.csv}.
\begin{table}[!ht]
\centering
\small
\caption{Included records (part 1/2): in-window, background anchors, and loop-transition. $A$=autonomy level; $B$=loop-position set; R/B/Ar = rubric scores; Acc = access status.}
\label{tab:apprecordsa}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lllllccc@{}}
\toprule
Record & Year & Unit & $A$ & $B$-set / role & Evidence class & R/B/Ar & Acc \\ \midrule
\multicolumn{8}{l}{\emph{In-window autonomy-bearing records (12)}}\\
...
ossfuzz-aifixing-2024 & 2024 & experiment & A2 (CodeRover-S) & B3 & experiment & 2/2/2 & PDF\\
...
csev12023bg & 2023 & benchmark & n/a & B1$^p$ & benchmark & 2/1/2 & PDF\\
intercode2023bg & 2023 & benchmark & n/a & B2$^p$ & benchmark & 3/2/3 & PDF\\
\midrule
\multicolumn{8}{l}{\emph{Loop-transition documentation (B4)}}\\
bloggoogle-summer2025 & 2025 & program-record & n/a & B4 — documents B1 discovery $\to$ announced B4 redeployment of Big Sleep to OSS projects & program-announcement & --/--/-- & HTML\\
openssfretro2026 & 2026 & program-record & n/a & B4 — documents post-competition B3$\to$B4 institutionalization (SIG hosting, transition funding, ecosystem stats) & foundation closeout & 1/1/2 & HTML\\
\bottomrule
\end{tabular}}
\end{table}
...
\centering
\small
\caption{Included records (part 2/2): off-axis, contextual, and out-of-window records. $A$=autonomy level; $B$=loop-position set; R/B/Ar = rubric scores; Acc = access status.}
\label{tab:apprecordsb}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lllllccc@{}}
\toprule
Record & Year & Unit & $A$ & $B$-set / role & Evidence class & R/B/Ar & Acc \\ \midrule
\multicolumn{8}{l}{\emph{General-SE comparators (security task not demonstrated)}}\\
autocoderoever-2024 & 2024 & system(experiment-benchmark) & n/a & context: general SE; security variant is CodeRover-S & benchmark/experiment & 3/3/3 & PDF\\
sweagent2024 & 2024 & system(experiment-benchmark) & n/a & context: general SE comparator (SWE-bench) & benchmark/experiment & 3/3/3 & PDF\\
\midrule
\multicolumn{8}{l}{\emph{Off-axis / contextual / out-of-window records}}\\
...
r3survey2026 & 2026 & survey & n/a & n/a & survey comparator (out-of-window) & 1/2/0 & PDF\\
...
r7sokaixcc2026 & 2026 & survey(systematization) & n/a & n/a & survey(systematization) & 2/3/2 & PDF\\
...
\centering
\small
\caption{Dated event chain (primary sources where available; secondary synthesis explicitly identified; grades per Sec.~\ref{sec:coevolution}).}
\label{tab:timeline}
\begin{tabular}{@{}p{2.0cm}p{15.0cm}@{}}
\toprule
Date & Event (source) \\
\midrule
2023-12 & AIxCC announced; ${\sim}\$29.5$M cumulative program commitment~\cite{darpa2023aixcc} \\
2024-08 & ASC: 42 teams $\rightarrow$ 7 finalists (\$2M each); Atlantis autonomously patches real SQLite FTS5 bug (unscored); official fix e9b919d5 follows~\cite{taasc2024} \\
2024-11 & Big Sleep reports SQLite stack-buffer-underflow zero-day; cites AIxCC inspiration (edge D1); AFL 150 CPU-h fails to rediscover; no CVE assigned~\cite{bigsleep2024}\\
2024-11 & OSS-Fuzz reports 26 vulns from AI-generated fuzz targets incl.\ CVE-2024-9143 (${\sim}$20 years latent)~\cite{ossfuzz-levelingup-2024}\\
2025-05 & DARKNAVY Argusee finds CVE-2025-37891 (Linux USB); root LPE by human team~\cite{darknavyargusee2025}\\
2025-06 & XBOW \#1 US HackerOne; ${\sim}$1060 submissions; mandated human review disclosed~\cite{xbowtop12025}\\
2025-07 & CVE-2025-6965 threat-intel-seeded disclosure; ``first time an AI agent has been used to directly foil'' claim~\cite{bloggoogle2025,nvd6965}\\
2025-08 & AFC finals: 48 challenges/23 repositories in the Trail of Bits account; 18 real vulns found; Atlanta \$4M / ToB \$3M / Theori \$1.5M; \$200K/team transition funding~\cite{darparesults2025,tobbuttercup2025,taafc2025,darpascoring2025}\\
2025-09 (detected), 2025-11 (reported) & Anthropic reports GTG-1002 campaign with an 80--90\% execution estimate; the incident remains a single-source, unresolved autonomy assessment~\cite{anthropicgtg2025,anthropicgtgreport2025}\\
$\sim$2026-02 & R7 AIxCC SoK preprint first released (v1--v5 through Aug 2026; final USENIX publication is outside the evidence cutoff); OpenSSF legacy retrospective: CRS SIG, FuzzingBrain 62v/26proj, OSS-CRS 25v/16proj~\cite{r7sokaixcc2026,openssfaixcc2026}\\
\bottomrule
\end{tabular}
\end{table}

% appendix-d.tex — A3 sensitivity analysis
\vspace{6pt}
\section{A3 Sensitivity Analysis}
\label{app:sensitivity}
Table~\ref{tab:sensitivity} reports counterfactual qualifying-system counts under explicitly
relaxed predicates. These scenarios do not change the paper's strict operational definition or
its bounded corpus finding.

\begin{table}[!ht]
\centering
\small
\caption{Counterfactual sensitivity analysis. Let $P$ denote independent planning/target
selection, $X$ execution, $V$ validation, $R$ reporting, and $E$ an E2 target environment
(Sec.~\ref{sec:framework}); the strict predicate is $P\land X\land V\land R\land E$.
S1--S3 relax exactly $R$, $P$, or $E$, respectively; S4 relaxes $P$, $R$, and $E$ together.
$X$ and $V$ remain mandatory in every scenario. Counts are counterfactual qualifiers, not
reassignments of the strict taxonomy.}
...
\midrule
S0 & None (baseline) & 0 & XBOW & XBOW: $R$; AIxCC: $P,E$;
ARTEMIS: $P$; GTG-1002: uncertain \\
S1 & $R$ & 1 & XBOW & AIxCC: $P,E$; ARTEMIS: $P$; GTG-1002: uncertain \\
S2 & $P$ & 1 & ARTEMIS & AIxCC: $E$; XBOW: $R$; GTG-1002: uncertain \\
S3 & $E$ & 0 & AIxCC CRS & AIxCC: $P$; XBOW: $R$; ARTEMIS: $P$ \\
S4 & $P,R,E$ & 3 & XBOW; AIxCC CRS; ARTEMIS & GTG-1002: uncertain \\
\bottomrule
\end{tabular}
\end{table}
