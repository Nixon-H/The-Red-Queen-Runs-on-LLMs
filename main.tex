...
\hypersetup{colorlinks=true,linkcolor=blue,urlcolor=blue,
  pdftitle={The Red Queen Runs on LLMs: Taxonomy, Evaluation, and Co-Evolution of LLM-Driven Cybersecurity},
...
in the offense/defense loop the system operates (B1 through B4). Fourteen records
received autonomy assessment. Pipeline automation (A1) already runs at ecosystem scale.
...
in SQLite that 150 CPU-hours of AFL could not rediscover~\cite{bigsleep2024}. A commercial
agent briefly held the top US position on a major bug-bounty platform~\cite{xbowtop12025}.
A government challenge scored seven fully autonomous systems on finding, proving, and
patching vulnerabilities in open-source software~\cite{darparesults2025,r7sokaixcc2026}.
Anthropic reported, as a single-source threat-intelligence assessment, an intrusion
campaign whose execution was 80--90\% autonomous~\cite{anthropicgtg2025}. These events
...
that produced them. GPT-4-based agents exploited 87\% of one-day CVEs when given their
descriptions and 7\% when not~\cite{fang2024agents}, yet the qualified number is the one
usually quoted.
...
phases. Thirteen in-window records receive definitive autonomy classifications; one remains
unresolved; the remaining 22 provide context, validity evidence, loop-transition
documentation, or positioning against prior work. We keep the distinction explicit throughout.
...
predetermined sequence rather than letting the model choose what to do next.
...
a critique Risse and B\"ohme systematized from measurement theory~\cite{rissebohme-wrongexam2024}. Ullah et
...
al.~\cite{iris2025} show that hybrid approaches combining LLM-assisted static analysis with
traditional pattern matching outperform either component alone on real-world datasets.
...
planning data structure for LLM-empowered automated penetration testing, showing that
domain-grounded agent architectures beat raw prompting on real-world
targets~\cite{pentestgpt2024}. The Model Context Protocol ecosystem, measured at
...
benchmarks across task complexity and domain-specific evaluation settings, finding a systemic
...
\midrule
R1 \cite{r1slr2024}        & Aug'24 & P & C & A & A & P\\
...
software engineering (e.g., SWE-agent, Agentless, AutoCodeRover) qualifies when the
evaluated artifact applies it to a security-specific task: vulnerability discovery, exploit
generation, or security patching. The autonomy level reflects the security evaluation, not
the system's general capability. AutoCodeRover, for example, is classified A2 because the
evaluated artifact applies it to OSS-Fuzz security bugs via CodeRover-S; the general
program-improvement system itself does not carry a security autonomy level. This rule ensures
...
First-page screening caps per query, publisher-API absence, and English-language sources are
 ...
 assigned to the 14 autonomy-assessed records. To check reproducibility, we
 repeated the classification for all 14 autonomy-assessed records, with six boundary
 cases (GTG-1002, XBOW, ARTEMIS, Big Sleep, Argusee, AIxCC cluster) getting additional
 blinded review during initial coding and a blind re-coding pass. The two passes yielded consistent
 ...
 assigned record's autonomy level. We report this as evidence that the taxonomy produces stable
 classifications when applied by the same coder (a within-coder stability check); a formal inter-rater
...
admits XBOW, while relaxing $P$ or $E$ alone admits none; relaxing $P$, $R$, and $E$ together
admits XBOW and the AIxCC CRS cluster under that altered rule. These are counterfactual
...
% tables/autonomy-loop-matrix.tex — frozen audit view (14 autonomy-assessed / 2 background /
...
ossfuzz-aifixing-2024    & 2024 & in & A2        & $\{B3\}$ \\
...
\bottomrule
\multicolumn{5}{l}{\scriptsize Totals: 14 autonomy-assessed ($A1{\times}2$, $A2{\times}11$, uncertain${\times}1$)
...
a human-curated pool of recent commits using Gemini~1.5~Pro~\cite{bigsleep2024}. Three
...
production-level blockchain implementations through code decomposition and adversarial
verification, a domain-specific demonstration that A2 task-agent patterns generalize beyond
...
reporting that agent frameworks exploit up to 13\% of critical-severity CVEs in
sandboxed environments under the benchmark's one-day setting, where a high-level vulnerability
...
Trail of Bits describes the scored structure as 48 challenges across 23 open-source
repositories in its participant account~\cite{tobbuttercup2025}. The later R7 systematization
counts the same 48 challenge projects across 24 repositories; we retain each figure within its
respective source and accounting scope~\cite{r7sokaixcc2026}. The CRSs also surfaced real,
...
and an explicit integration taxonomy (LLM-augmented, LLM-opinionated, LLM-driven) with
...
using exclusively non-reasoning models, achieving lower reported cost per competition point at
...
multi-agent, and general-purpose code agents. On the security-specific repair side, tree-of-thought prompting with
...
vulnerabilities without test inputs or exploit evidence, achieving up to 28.33\% F1 improvement
over non-LLM baselines on 97 zero-day vulnerabilities. These systems establish that B3 capability now
...
LLM-based bug-fixing systems, showing that fault localization and the interaction between issue
reproduction and repair as major determinants of repair success, indicating that additional
agentic interaction does not uniformly improve outcomes, a finding with direct implications
...
created transition incentives of up to \$200K per team to integrate CRSs into critical software~\cite{darparesults2025};
...
into critical software~\cite{darparesults2025}. Team Atlanta recycled half of its \$4M prize
...
conditions~\cite{fang2024agents}. This more-than-an-order-of-magnitude difference is produced by a single
supplied-knowledge variable, the CVE text acts as a partial answer key, so any
...
LLM-based filtering can reduce initial false-positive rates from over 92\% to as low as 6.3\%
in the best configuration, though aggressive filtering can itself suppress true
...
this evaluation. PrimeVul's reconstruction moves a
...
mechanistic analysis by Sandoval et al.~\cite{surgicalrepair2026} reveals a
...
with them, suggesting that insecure code generation is an interpretability problem,
not a training deficit. Shahriar et
...
and anchoring---systematically affect LLM vulnerability
 detection, with models disproportionately flagging vulnerabilities in code they have seen
 frequently in training data while missing novel patterns. Each datum predates or
 ...
 models, while Dahiya et al.~\cite{frontierllmsready2026} report that Gemini 3.1 Pro reaches
 81.3\% recall on white-box function-level vulnerability classification, while black-box
 ground-truth coverage remains only 4--8\% without methodology guidance, illustrating how
 benchmark performance can overstate real-world capability when methodology and tooling
...
\textbf{Single-corpus induction.} All quantitative distributions (14 autonomy-assessed records,
including 13 classified and one unresolved; per-cell counts) derive from one team's inclusion decisions under one protocol; the saturation
...
version, Lee et al.~\cite{secbenchpro2026} evaluate 183 validated vulnerabilities across V8 and
SpiderMonkey, with the strongest frontier configuration reaching 32.0\% on V8 and 38.8\% on
SpiderMonkey, while the open-weight Kimi-K2.6 baseline reaches 11.7\% on V8. The two-agent
union reaches 37.9\% on V8 and 48.8\% on SpiderMonkey.
...
on a 36-record multi-stream corpus of which 14 records underwent autonomy assessment (13
definitive classifications, one unresolved), is that
...
of up to \$200K per finalist team for transition activities, while the competition itself carried prize commitments reported
as \$29.5M in DARPA's 2023 announcement and \$30.5M in OpenSSF's later retrospective; separately,
OpenSSF established governance and hosting structures for post-competition CRS development; and
...
Information conditions change reported exploitation rates by more than an order of magnitude; dataset
reconstruction collapses detection SOTA; oracle gaming has a named taxonomy; environment-specific
heuristics can materially affect system outcomes; and leaderboard counting conflates outcome categories. Because
...
methodological constraint on what this field can claim to know, pending the
decision-impact audit (E7) that would confirm this.
...
\centering
\small
\caption{Included records (part 2/2): off-axis and context records. $A$=autonomy level; $B$=loop-position set; R/B/Ar = rubric scores; Acc = access status.}
...
Date & Event (source) \\
\midrule
2023-08 & AIxCC announced; ${\sim}\$29.5$M cumulative program commitment~\cite{darpa2023aixcc} \\
...
2026-02 & R7 AIxCC SoK preprint first released; later August 2026 publication is outside the evidence cutoff; OpenSSF legacy retrospective: CRS SIG, FuzzingBrain 62v/26proj, OSS-CRS 25v/16proj~\cite{r7sokaixcc2026,openssfaixcc2026}\\
\bottomrule
\end{tabular}
\end{table}
...
ARTEMIS: $P,V$; GTG-1002: uncertain \\
...
\vspace{2pt}
\noindent\textbf{Data and Code Availability:} The paper source, bibliography, evidence corpus, and classification data are available at:
\url{https://github.com/Nixon-H/The-Red-Queen-Runs-on-LLMs}.
The repository includes the \texttt{evidence/autonomy-loop-assignments.csv} artifact
containing record-level autonomy classifications and the complete bibliography tracking
files referenced throughout this paper.
