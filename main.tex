...
evidence met our operational definition of an autonomous production hunter (A3). We include
...
First-page screening caps per query, publisher-API absence, and English-language sources are
 documented limitations; the A3-negative finding remains robust within the documented corpus under the sensitivity scenarios reported in Sec.~\ref{sec:framework}, but cannot exclude undocumented systems outside the observable evidence base.
 Classification reliability: all 36 records were classified by the first author using the
 autonomy$\times$loop taxonomy (Sec.~\ref{sec:framework}). To assess reproducibility, we
 ...
 boundaries (E1 vs.\ E2) and the unresolved classification of GTG-1002, which the
 weakest-missing-condition rule (Sec.~\ref{sec:framework}) resolved without changing any
 record's autonomy level. We report this as evidence that the taxonomy produces stable
...
no-human-interaction corollary ensures that systems with human review in their reporting
pipeline (e.g., XBOW) remain non-A3 even when the human-review condition is relaxed. The
...
gtg1002-2025             & 2025 & in & unresolved & $\{B1{+}B2\}$ \\
...
\hline
\multicolumn{5}{l}{\scriptsize Totals: 14 autonomy-assessed ($A1{\times}2$, $A2{\times}11$, unresolved${\times}1$, $A3{=}0$)
...
The ARTEMIS study (arXiv preprint December 2025) places six agent instances against ten
professionals on the same corporate-style network (${\sim}8{,}000$ hosts, 12 subnets), scoring the
...
Participant reporting describes the scored structure as 48 challenges across 23 open-source
repositories in the final round~\cite{tobbuttercup2025}, operated over what the
...
2025-08 & AFC finals: 48 challenges/23 repositories; 143 h fully autonomous operation; 18 real vulns found; Atlanta \$4M / ToB \$3M / Theori \$1.5M; \$200K/team transition funding~\cite{darparesults2025,tobbuttercup2025,taafc2025,darpascoring2025}\\
...
Table~\ref{tab:sensitivity} reports the A3 count under progressively relaxed classification
rules. All scenarios yield zero qualifying systems under the paper's strict operational
definition.

\begin{table*}[t]
\centering
\caption{A3 sensitivity analysis. A3 requires independent planning, execution, validation,
and reporting in environment class E2 (Sec.~\ref{sec:framework}). The \emph{no-human-interaction
corollary} requires that reporting be fully autonomous; this corollary is not relaxed in any
scenario. Each row relaxes one condition while holding the others fixed.}
\label{tab:sensitivity}
\scriptsize
\begin{tabular}{lllll}
\hline
Scenario & Condition relaxed & A3 & Closest candidate & Remaining blocker(s) \\
\hline
S0 & None (baseline) & 0 & XBOW & XBOW: human review (reporting); AIxCC: targets + E1;
ARTEMIS: human scope (planning); GTG-1002: unresolved \\
S1 & Human review & 0 & XBOW & XBOW: corollary (human review in pipeline);
AIxCC: targets + E1; ARTEMIS: human scope + review \\
S2 & Target selection & 0 & AIxCC CRS & AIxCC: E1 environment; XBOW: corollary;
ARTEMIS: human scope + review \\
S3 & E2 environment & 0 & AIxCC CRS & AIxCC: human-selected targets; XBOW: corollary;
ARTEMIS: human scope + review \\
S4 & All combined & 0 & XBOW & XBOW: corollary; ARTEMIS: human scope + review;
AIxCC: E1; GTG-1002: unresolved \\
\hline
\end{tabular}
\end{table*}
