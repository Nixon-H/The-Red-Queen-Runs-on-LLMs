...
and the offense/defense loop positions B1--B4. Fourteen in-window records underwent autonomy
assessment: pipeline automation (A1) already operates at ecosystem scale; task agents (A2)
...
evidence met our operational definition of A3. We further grade offense$\leftrightarrow$
...
evaluation validity as a measured methodological constraint: information conditions change reported exploitation
rates by more than an order of magnitude, dataset reconstruction collapses detection performance, and proof-of-
...
150 CPU-hours of AFL could not rediscover~\cite{bigsleep2024}; a commercial agent
...
reported (as a single-source threat-intelligence assessment) an intrusion campaign whose execution was 80--90\% autonomous~\cite{anthropicgtg2025}.
...
We build a PRISMA-informed, multi-stream evidence base (academic literature; competition and
...
phases. Thirteen in-window records receive definitive autonomy classifications; one additional
incident remains unresolved; the remaining 22 provide
...
\textbf{I3.} Evaluation validity \emph{appears to be} a binding methodological constraint on what this field can
claim to know --- pending the decision-impact audit (E7) that would confirm or refute this assessment.
...
predetermined sequence rather than allowing the model to dynamically select subsequent actions.
...
their security-relevant evaluations began with secure-coding benchmarks~\cite{csev12023bg} and
...
a critique systematized by Risse and B\"ohme~\cite{rissebohme-wrongexam2024}. \emph{Interactive
...
covers detection-to-remediation with no exploitation axis~\cite{r2slr2024}; R3 ---
...
software security~\cite{r4csur2025}. R5 systematizes the security of agent systems ---
...
November 2025 but no offense axis and no industrial track~\cite{r6tosem2026}. R7, the AIxCC
...
R4 \cite{r4csur2025}        & '26    & A & C & A & A & A\\
...
software engineering (e.g., SWE-agent, Agentless, AutoCodeRover) is eligible when the
...
the system's general capability. For example, AutoCodeRover is classified A2 because the
evaluated artifact applies it to OSS-Fuzz security bugs via CodeRover-S; the general
program-improvement system itself does not bear a security autonomy level. This rule ensures
that the corpus captures the security-relevant frontier without conflating general SWE
...
behavioral evidence is published in the artifact (\texttt{evidence/autonomy-loop-assignments.csv});
...
First-page screening caps per query, publisher-API absence, and English-language sources are
 documented limitations; the bounded A3 finding remains robust within the documented corpus under the sensitivity scenarios reported in Sec.~\ref{sec:framework}, but cannot exclude undocumented systems outside the observable evidence base.
 Classification reliability: all 36 records were coded for unit type, temporal scope, access,
 evidence, and loop role; the autonomy$\times$loop taxonomy (Sec.~\ref{sec:framework}) was
 assigned to the 14 autonomy-assessed records. To assess reproducibility, we
 repeated the classification procedure for all 14 autonomy-assessed records, with six boundary
 ...
 classifications, although intermediate uncertainty was concentrated on environment-class
 boundaries (E1 vs.\ E2); GTG-1002 remains unresolved because the available incident
 documentation is insufficient to assign a definitive autonomy level. The
 ...
 classifications when applied by the same coder; a formal inter-rater reliability study with
 independent domain-expert coders remains future work. Additional methodological constraints include:
...
empty under the operational definition; B4 exists only as loop-transition documentation tied to
 documented systems. Counts are reconciled against the per-record YAML evidence files and the
 assignment CSV; the table is a frozen audit view rather than a generated data product.
...
% CP2-lock schema (14 autonomy-assessed / 2 background / 2 loop-transition-doc / 18 context).
...
r4csur2025 (R4)       & 2026 & in & n/a & survey (paywalled; metadata) \\
...
capability from scaffold and task conditions.
...
security benchmarks. CyberGym~\cite{cybergym2026} further scales evaluation to real-world
...
repositories in the final round~\cite{tobbuttercup2025}; the R7 systematization instead counts
48 scored challenge projects drawn from 24 OSS-Fuzz repositories~\cite{r7sokaixcc2026}. This is
a source-level discrepancy in the repository denominator, not a reason to merge the counts. The
final round operated over what the
systematization records as ``143 hours of fully autonomous operation'' by CRSs from
...
These systems establish that B3 capability now
spans from pipeline-orchestrated repair (Agentless) through iterative agentic workflows
(AutoCodeRover, CodeRover-S), yet robust
validation of semantic security correctness beyond replaying known exploit inputs remains
unevenly evaluated across the literature.
...
Four gaps bound current evidence. (i)~\emph{Semantic exploit-aware validation}: several
...
primaries), and explicit CLAIMED labels --- but asymmetry remains, and one unresolved,
vendor-reported incident (GTG-1002) materially shapes the autonomy frontier of our corpus.
...
decisions through transition programs --- evaluation validity \emph{appears to be} a binding
methodological constraint on what this field can claim to know, though the decision-impact
audit (E7) has not yet been conducted to confirm this assessment.
...
r4csur2025 & 2026 & survey & n/a & n/a & survey & 0/0/0 & payw.\\
...
\label{app:timeline}
\begin{center}
\captionof{table}{Dated event chain (primary sources where available; secondary synthesis explicitly identified; grades per Sec.~\ref{sec:coevolution}).}
\label{tab:timeline}
\centering\scriptsize
\resizebox{\columnwidth}{!}{%
...
2025-08 & AFC finals: 48 challenges/23 repositories in the Trail of Bits account (R7 counts 24 OSS-Fuzz repositories); 143 h fully autonomous operation; 18 real vulns found; Atlanta \$4M / ToB \$3M / Theori \$1.5M; \$200K/team transition funding~\cite{darparesults2025,tobbuttercup2025,taafc2025,darpascoring2025}\\
...
2026-02 & R7 AIxCC SoK preprint released; 2026-08 USENIX Security 26 publication; OpenSSF legacy retrospective: CRS SIG, FuzzingBrain 62v/26proj, OSS-CRS 25v/16proj~\cite{r7sokaixcc2026,openssfaixcc2026}\\
\hline
\end{tabular}}
\end{center}
